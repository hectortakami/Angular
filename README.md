# Angular 10+ & MEAN Stack

TypeScript + HTML + CSS toolset for building Progressive Web Applications (PWA). https://angular.io/cli
...
```console
$ ng generate {component | service | pipe | guard | module} [creation_path_and_filename] [--help]? [--dry-run]? [--skipTests]?
```

### Deploy Angular App (PWA Server)
...
2. Compile project to production

   _Note: All variables in `environment.ts` file will be replaced with `environment.prod.ts`, also the TS classes & modules will be compile to JS._
